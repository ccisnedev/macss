...
1. Establish the scope from the issue and its acceptance criteria. If the issue
   is not specified yet, you are in the wrong stage — go back to
   `macss specification`.
...
4. Name the risks and dependencies: what could break, what must land first,
   what is uncertain.
5. Write the diagnosis. State the problem, the evidence, the affected layers,
   the risks, and the open questions. It is a file on disk, not your memory.
6. Present it to the human and get agreement on the problem before planning.
...
is installed, it drives the same loop with enforced gates and scaffolds the
diagnosis artifact for you — `iq fsm state --json` will tell you where you are
and what it expects. This skill is written to stand on its own without it.
